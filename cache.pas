...
type
 ...
 TCacheItem=record
  ...
  cells:TField;
  rate:single;
  quality:word; // если больше 0 - оценка из БД; если 0 - обычная оценка
   ...
   if CompareMem(@b.cells,@rateCache[h].cells,sizeof(TField)) then
    with b do begin
     ...
     if rateCache[h].quality>0 then begin
      // Оценка из БД
      // Достаточный ли показатель качества для данного уровня глубины?
      // Нет смысла писать оценки с низким качеством, например, в 1-й слой, т.к.
      // придётся развивать почти сразу же.
      result:=false;
      q:=rateCache[h].quality;
      d:=b.depth-curBoard.depth;
      case d of
       1:if q<20000 then exit;
       2:if q<2000 then exit;
      end;
      // Качество оценок из БД обычно значительно выше простых соседних узлов.
      // Нужно записать некое промежуточное значение, которое, с одной стороны,
      // не навредит развитию предка, а с другой - отразит качество самой оценки.
      q:=round(q*0.2);
      qFromDB:=q;
      quality:=q;
      SetFlag(movDB);
